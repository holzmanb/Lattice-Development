...
def getMinimaxedMove(board,computerLetter):
    ...
    for move in allMoves:
        ...
        score = minimaxRecursion(tempBoard, playerLetter, computerLetter, 5, alpha, beta, move)
        if score > bestScore:
            bestScore = score
            alpha = bestScore
            bestMove = move
...
def minimaxRecursion(board, currentLetter, maxLetter, depth, alpha, beta, move):
    global computed
    computed+= 1
    ...
    if (len(otherBoxes[3])):
        ...
        for box in otherBoxes[3]:
            for point in box:
                if board[point] == ' ':
                    moves.append(point)
        return getBestScore(moves, board, currentLetter, maxLetter, depth, alpha, beta, move)
    ...
    if (len(currentBoxes[2])):
        ...
        possible_scores.append(getBestScore(moves, board, currentLetter, maxLetter, depth - 1, alpha, beta, move))
    if computed > 500000:
        print("oh god", move)
        possible_scores.append(boardHeuristic(currentBoxes, otherBoxes, sign))
    else:
        print(depth)
        possible_scores.append(getBestScore(allMoves, board, currentLetter, maxLetter, depth - 2, alpha, beta, move))
...
def getBestScore(moves, board, nextLetter, maxLetter, recursionDepth, alpha, beta, last_move):
    ...
    for move in moves:
        ...
        score = minimaxRecursion(tempBoard, otherLetter, maxLetter, recursionDepth, alpha, beta, last_move)
